...
public class Parser {
	...
	public ElementGroup elementGroup = new ElementGroup();
	public ArrayList<SimpleType> simpleTypeList = new ArrayList<SimpleType>();
	public ArrayList<ComplexType> complexTypeList = new ArrayList<ComplexType>();
	...
	public void parseSchema() {
		try {
			...
			Iterator<XSComplexType> complexTypeItr = globalSchema
					.iterateComplexTypes();
			...
			for (int i = 0; i < complexTypeList.size(); i++)
				System.out.println(complexTypeList.get(i).getComplexTypeName()
						+ " -- " + complexTypeList.get(i).getAttributeName()
						+ " -- " + complexTypeList.get(i).getAttributeType());
			...
			for (int i = 0; i < simpleTypeList.size(); i++)
				System.out.println(simpleTypeList.get(i).getTypeName() + " -- "
						+ simpleTypeList.get(i).getPatternRestriction()
						+ " -- " + simpleTypeList.get(i).getPatternValue());
					...
					for (XSParticle p : particles) {
						...
						if (pterm.isElementDecl()) {
							...
							ElementType elementType = new ElementType(
									ee.getName(), getElementType(ee),
									ed.getName());
							elementsList.add(elementType);

							if (pterm.asElementDecl().getName()
									.equals(elemName)) {
								XSContentType content = ee.getType()
										.asComplexType().getContentType();
								XSParticle particle1 = content.asParticle();
								getInternElements(particle1, elementsList,
										parentElementName);
							}
						}
					}
			...
			for (int i = 0; i < elementsList.size(); i++)
				System.out.println(elementsList.get(i).getParentElementName()
						+ " -- " + elementsList.get(i).getElementName()
						+ " -- " + elementsList.get(i).getElementType());
			System.out.println("\nAfisare parametrii:");
			for (int i = 1; i < elementsList.size(); i++) {
				if (elementsList.get(i).getParentElementName()
	...
	private void getInternElements(XSParticle particle,
			ArrayList<ElementType> elementsList, String elemName) {
		if (particle != null) {
			XSTerm term = particle.getTerm();
			if (term.isModelGroup()) {
				XSModelGroup xsModelGroup = term.asModelGroup();
				...
				for (XSParticle p : particles) {
					XSTerm pterm = p.getTerm();
					if (pterm.isElementDecl()) {
						ElementType elementType = new ElementType(pterm
								.asElementDecl().getName(),
								getElementType(pterm.asElementDecl()), elemName);
						elementsList.add(elementType);
					}
				}
			}
	...
	private SimpleType getSimpleType(XSSimpleType simple) {
		...
		if (restriction != null) {
			...
			if (restriction.getDeclaredFacets().size() == 0) {
				simpleType = new SimpleType(restriction.getName(), restriction
						.getBaseType().getName(), null);
				return simpleType;
			}
			XSFacet facet = i.next();
			simpleType = new SimpleType(restriction.getName(), restriction
					.getBaseType().getName(), facet.getValue().value);
			return simpleType;
		}
		return null;
	}
	...
	private ComplexType getComplexType(XSComplexType complex) {
		...
		complexType = new ComplexType(complex.getName(),
				attributeDecl.getName(), attributeDecl.getType().getName());
		return complexType;
	}
}